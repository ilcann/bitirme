...
import type { CourseAttendanceStudent } from '@/services/types';
...
export const useCourseAttendancePagination = (students: CourseAttendanceStudent[]) => {
    ...
    const totalPages = Math.max(1, Math.ceil(filteredStudents.length / ATTENDANCE_PER_PAGE));
